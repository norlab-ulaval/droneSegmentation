...
log_file_path = lac_dir / "Final_iNat_Classifier/log_final_s8.txt"
u.setup_logging("final_s8", log_file_path)
logger = logging.getLogger("final_s8")
...
train_transform = Compose(
    [
        CustomGaussianBlur(sigma=8, p=1.0),
...
for fold, (train_idx, val_idx) in enumerate(kf.split(dataset, dataset.targets)):
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            pth_name = f"5_8{fold + 1}_Final_time{t}_{epoch + 1}e_acc{100 * accuracy:2.0f}.pth"
            torch.save(
                model_weights,
            ...
            if accuracy_valid > best_accuracy:
                ...
                pth_name = f"5_8{fold + 1}_Final_time{t}_best_{epoch + 1}e_acc{100 * accuracy:2.0f}.pth"
                torch.save(
                    model_weights,
                    checkpoint_dir / pth_name,
                )
