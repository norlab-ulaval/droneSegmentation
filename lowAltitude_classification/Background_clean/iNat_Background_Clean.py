...
log_file_path = "/lowAltitude_classification/filtered_iNat_classifier/log_background.txt"
...
for fold, (train_idx, val_idx) in enumerate(kf.split(dataset, dataset.targets)):
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            pth_name = f"4{fold + 1}_background_time{t}_{epoch + 1}e_acc{100 * accuracy:2.0f}.pth"
            torch.save(
                model_weights,
            ...
            if accuracy_valid > best_accuracy:
                ...
                pth_name = f"4{fold + 1}_background_time{t}_best_{epoch + 1}e_acc{100 * accuracy:2.0f}.pth"
                torch.save(
                    model_weights,
                    checkpoint_dir / pth_name,
                )
