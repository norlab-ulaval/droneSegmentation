...
def add_images_to_map(images_directory):
    ...
    for filename in os.listdir(images_directory):
        image = np.array(Image.open(os.path.join(images_directory, filename)).convert('L'))
...
pseudo_labels_pixel_percentage = pseudo_labels_class_counts / np.sum(pseudo_labels_class_counts)
...
sorted_latin_names = [('\n'.join(common_to_latin_mapping[name].split()) if all(len(word) > 7 for word in common_to_latin_mapping[name].split()) else common_to_latin_mapping[name]) if name in common_to_latin_mapping.keys() else name for name in sorted_class_labels]
sorted_latin_names = [i if i != "Background" else "Other" for i in sorted_latin_names]

plt.figure(figsize=(16, 10), dpi=300)
plt.bar(indices - bar_width, sorted_pseudo_labels_pixel_percentage, width=bar_width, color='green', label='Pseudo-labels (with voting)')
plt.bar(indices, sorted_trainval_pixel_percentage, width=bar_width, color='blue', label='Train/val (annotations)')
plt.bar(indices + bar_width, sorted_test_pixel_percentage, width=bar_width, color='red', label='Test (annotations)')
