...
for metric in ['Accuracy', 'F1 Score - Macro', 'F1 Score - Weighted']:
    ...
    plt.hist(split_overlap_data[split][0][metric], bins=10, alpha=0.5, color='blue', label='Overlap 0')
    plt.hist(split_overlap_data[split][0.2][metric], bins=10, alpha=0.5, color='orange', label='Overlap 0.2')
    # line for mean
    plt.axvline(np.mean(split_overlap_data[split][0][metric]), color='blue', linestyle='dashed', linewidth=1)
    plt.axvline(np.mean(split_overlap_data[split][0.2][metric]), color='orange', linestyle='dashed', linewidth=1)
    plt.legend()
    plt.show()
