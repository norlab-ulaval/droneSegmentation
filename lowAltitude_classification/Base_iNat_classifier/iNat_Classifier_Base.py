    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset, dataset.targets)):
        ...
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                ...
                log_file.write(f'Validation Loss: {loss_accumulated_valid / total_samples_valid:.4f} Accuracy: {accuracy_valid:.4f}\n')

                accuracy = accuracy_valid
                model_weights = model.state_dict()
                t = datetime.date.today()
                pth_name = f"0{fold + 1}_base_time{t}_{epoch + 1}e_acc{100 * accuracy:2.0f}.pth"
                ...
                if accuracy_valid > accuracy:
                    pth_name = f"0{fold + 1}_base_time{t}_best_{epoch + 1}e_acc{100 * accuracy:2.0f}.pth"
                    torch.save(model_weights,
                               f'/home/kamyar/PycharmProjects/droneSegmentation/lowAltitude_classification/checkpoints/{pth_name}')
                    log_file.write(f"Best model weights saved for fold {fold + 1} at epoch {epoch + 1} with accuracy {accuracy:.4f}\n")
