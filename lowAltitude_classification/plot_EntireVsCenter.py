...
filtered_val = merged_val_df[(merged_val_df['Pad Size'] == 184) & (merged_val_df['Step Size'] == 27)]
filtered_test = merged_test_df[(merged_test_df['Pad Size'] == 184) & (merged_test_df['Step Size'] == 27)]
...
bars1 = ax1.bar(filtered_val['Central Size'], filtered_val.F1, color='teal', width=16, edgecolor='black')
...
for bar in bars1:
    yval = bar.get_height()
    ax1.text(bar.get_x() + bar.get_width()/2, yval + 0.07, f'${round(yval, 2)}$', fontsize='x-small', color='darkgreen', ha='center', va='bottom')

bars2 = ax2.bar(filtered_test['Central Size'], filtered_test.F1, color='teal', width=16, edgecolor='black')
...
for bar in bars2:
    yval = bar.get_height()
    ax2.text(bar.get_x() + bar.get_width()/2, yval + 0.07, f'${round(yval, 2)}$', fontsize='x-small', color='darkgreen', ha='center', va='bottom')
