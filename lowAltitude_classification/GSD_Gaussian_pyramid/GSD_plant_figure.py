...
for i, (filename, gsd_label) in enumerate(zip(image_files, sizes_values)):
    ...
    blurred_patch = blurred_image.crop(
        (blurred_image.width - 256, 0, blurred_image.width, 256)
    )
    ...
    size = 256 // (2**i)
    downsampled_patch = downsampled_image.crop(
        (downsampled_image.width - size, 0, downsampled_image.width, size)
    )
...
axs[0, 0].text(
    0.5, 1.03, "Original image", fontsize=24, ha="center", transform=axs[0, 0].transAxes
)
