...
from albumentations import (
    ...
    Compose, CenterCrop, ColorJitter, Blur, RandomBrightnessContrast, ShiftScaleRotate
)
...
log_file_path = "/home/kamyar/PycharmProjects/droneSegmentation/lowAltitude_classification/Augmentation_iNat_classifier/log_aug22.txt"
...
with open(log_file_path, 'w') as log_file:
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset, dataset.targets)):
        ...
        for epoch in range(num_epochs):
            ...
            log_file.write(f'Epoch {epoch + 1} - Training Loss: {loss_accumulated_train / total_samples_train:.4f} Accuracy: {correct_predictions_train / total_samples_train:.4f}\n')
            scheduler.step()
            ...
            with torch.no_grad():
                ...
                pth_name = f"22{fold + 1}_aug2_time{t}_{epoch + 1}e_acc{100 * accuracy:2.0f}.pth"
                ...
                if accuracy_valid > accuracy:
                    pth_name = f"22{fold + 1}_aug2_time{t}_best_{epoch + 1}e_acc{100 * accuracy:2.0f}.pth"
                    torch.save(model_weights,
                               f'/home/kamyar/PycharmProjects/droneSegmentation/lowAltitude_classification/checkpoints/{pth_name}')
                    log_file.write(
        ...
        log_file.write(f"Average validation accuracy across folds: {np.mean(fold_accuracies):.4f}\n")
