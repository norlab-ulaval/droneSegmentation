...
def write_dataset(images, masks, out):
    ...
    with mp.Pool(64) as p:
        ...
        it = tqdm.tqdm(
            p.imap(functools.partial(copy_to, dst=out_images), images),
            total=len(images),
        )
        deque(it, maxlen=0)
        print("Copying masks...")
        it = tqdm.tqdm(
            p.imap(functools.partial(copy_to, dst=out_masks), masks), total=len(masks)
        )
        deque(it, maxlen=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--full_root", type=str, required=True, help="Path to the full dataset"
    )
    parser.add_argument(
        "--out", type=str, required=True, help="Path to the output directory"
    )
    parser.add_argument(
        "--split", type=str, required=True, help="Split factor (e.g. 1/2, 1/4, 1/8)"
    )
    ...
    print(
        f"{len(subset_images)} images and {len(subset_masks)} masks will be copied to {out}"
    )
